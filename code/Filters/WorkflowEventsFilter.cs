...
{
  class WorkflowEventsFilter : Sitecore.Feature.Reports.Interface.BaseFilter
  {
    ...
    public override bool Filter(object element)
    {
      if (element is ItemWorkflowEvent)
      {
        ...
        int count = (from i in StateTransitions
                     where i.OldState == iwe.WorkflowEvent.OldState && i.NewState == iwe.WorkflowEvent.NewState
                     select i).Count();
        if (count > 0)
        {
          return true;
