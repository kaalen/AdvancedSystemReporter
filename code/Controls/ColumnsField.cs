...
namespace Sitecore.Feature.Reports.Controls
{
		...
		protected void Edit(ClientPipelineArgs args)
		{
			...
			if (Enabled)
			{
				if (args.IsPostBack)
				{
					...
					{
						XmlDocument doc = XmlUtil.LoadXml(WebUtil.GetSessionString("Sitecore.Feature.Reports.ColumnEditor"));
						WebUtil.SetSessionValue("Sitecore.Feature.Reports.ColumnEditor", null);
				...
				else
				{
					...
					WebUtil.SetSessionValue("Sitecore.Feature.Reports.ColumnEditor", document.OuterXml);
					...
					handle["value"] = value;
				    handle["id"] =
				        new ItemUri(Sitecore.Data.ID.Parse(ItemID), Language.Parse(ItemLanguage), Version.Parse(ItemVersion),
