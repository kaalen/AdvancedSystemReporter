namespace Sitecore.Feature.Reports.DomainObjects
{
  ...
  public class ReportItem : BaseItem
  {
    ...
    public IEnumerable<ScannerItem> Scanners
    {
      get
      {
        return _scanners ?? (_scanners = GetMultilistField("scanners").Select(i => new ScannerItem(i)).ToArray());
      }
    }
    ...
    public IEnumerable<ViewerItem> Viewers
    {
      get
      {
        return _viewers ?? (_viewers = GetMultilistField("viewers").Select(i => new ViewerItem(i)).ToArray());
      }
    }
    ...
    public IEnumerable<CommandItem> Commands
    {
      get
      {
        return _commands ?? (_commands = GetMultilistField("commands").Select(i => new CommandItem(i)).ToArray());
      }
    }
    ...
    public IEnumerable<FilterItem> Filters
    {
      get
      {
        return _filters ?? (_filters = GetMultilistField("filters").Select(i => new FilterItem(i)).ToArray());
      }
    }
