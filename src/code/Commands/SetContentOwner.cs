...
{
  [Serializable]
  public class SetContentOwner : Command
  {
    ...
    protected void Run(ClientPipelineArgs args)
    {
        ...
        if (args.HasResult)
        {
          ...
          foreach (var uri in uris)
          {
            if (ItemUri.IsItemUri(uri))
            {
              ...
              item["content owner"] = result;
              item.Editing.EndEdit();
              Log.Audit(this, "Set content owner: {0}", AuditFormatter.FormatItem(item));
            }
          }
        }
      }
      else
      {
        if (ItemUri.IsItemUri(uris[0]))
        {
          ...
          UrlString str6 = new UrlString("/sitecore/shell/~/xaml/Sitecore.Feature.Reports.SetContentOwner.aspx");
