...
namespace Sitecore.Feature.Reports.Scanners
{
  ...
  /// <summary>
	/// Scans Sitecore log files and returns log entries which are newer than 
	/// the age parameter.
	/// </summary>
	public class LogScanner : Sitecore.Feature.Reports.Interface.BaseScanner
  {
    ...
    protected string Valid_Types
    {
      get
      {
        if (valid_types == null)
        {
          valid_types = this.GetParameter(ENTRY_TYPES_PARAMETER);
    ...
    protected int Age
    {
      get
      {
        ...
        {
          if (!int.TryParse(this.GetParameter(AGE_PARAMETER), out _age))
            _age = -1;
        }
        return _age;
      }
