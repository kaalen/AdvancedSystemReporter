...
using System.Text;
using System.Web.UI;
using Sitecore.Feature.Reports.Controls;
...
namespace Sitecore.Feature.Reports
{
  ...
  public class MainFormNew : BaseForm, IHasCommandContext
  {
    ...
    {
      if (message.Name == "reports:updateSection")
      {
      ...
      if (message.Name.StartsWith("reports:"))
      {
    ...
    [HandleMessage("reports:openlink")]
    ...
    private bool MakeControls(ReferenceItem referenceItem)
    {
      ...
      var panel = new Panel();
      //panel.Style.Add("border", "1px solid red");
      panel.Style.Add("margin-bottom", "10px");

      var literal = new Literal
      {
        Text = string.Format("<div style=\"margin-left:10px;margin-top:4px;font-weight:bold\">{0}</div><br/>",
      ...
      foreach (var parameter in referenceItem.Parameters)
      {
        var parameterPanel = new Panel();
        var label = new Label { Header = parameter.Title + ":" };

        label.Style.Add("font-weight", "bold");
        label.Style.Add("padding-top", "5px");
        label.Style.Add("margin-right", "10px");
        label.Style.Add("margin-left", "20px");
        label.Style.Add("width", "100px");
        label.Style.Add("text-align", "right");
        label.Style.Add("float", "left");

        var input = parameter.MakeControl();
        input.Style.Add("display", "inline-block");
        label.For = input.ID;

        parameterPanel.Style.Add("display", "block");
        parameterPanel.Style.Add("margin-top", "5px");
        parameterPanel.Value = input.ID;
        parameterPanel.ID = Control.GetUniqueID("params_" + parameter.Name + "_");
        parameterPanel.Controls.Add(label);
        parameterPanel.Controls.Add(input);
        //var lit = new Literal { Text = "<br/>" };
        //i.Controls.Add(lit);
        panel.Controls.Add(parameterPanel);
      }
    ...
    private IPageable<DisplayElement> GetDisplayElements()
    {
      var results = Current.Context.Report.GetResultElements();
      if (results == null)
      {
        return Assert.ResultNotNull<Pageable<DisplayElement>>(Pageable<DisplayElement>.Empty);
      }
      return new Pageable<DisplayElement>((int pageIndex, int pageSize) => results.Skip(pageIndex * pageSize).Take(pageSize), () => results, () => results.Count());
    }

    private void PopulateItemGrid()
    {
      IPageable<DisplayElement> results= this.GetDisplayElements();
      ComponentArtGridHandler<DisplayElement>.Manage(this.Items, new GridSource<DisplayElement>(results), true/*!XamlControl.AjaxScriptManager.IsEvent*/);
    }
    ...
    private void PopulateItemList(int start, int count)
    {
      ...
      var noPages = (int)Math.Ceiling((decimal)Current.Context.Report.ResultsCount() / Current.Context.Settings.PageSize);
    ...
    [HandleMessage("Sitecore.Feature.Reports.MainForm:runfinished", false)]
    ...
    [HandleMessage("Sitecore.Feature.Reports.MainForm:update", false)]
    ...
    [HandleMessage("Sitecore.Feature.Reports.MainForm:updateparameters", false)]
    [HandleMessage("Sitecore.Feature.Reports:linkit")]
    private void UpdateParameters(Message message)
    {
      ...
      if (message.Name == "Sitecore.Feature.Reports:linkit")
      {
        Sitecore.Context.ClientPage.SendMessage(this, "Sitecore.Feature.Reports:createlink");
      }
    }
